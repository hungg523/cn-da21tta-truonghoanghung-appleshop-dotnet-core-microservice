...
{
    public class GetAllProductsActivedQueryHandler : IRequestHandler<GetAllProductsActivedQuery, Result<List<ProductFullDTO>>>
    {
        ...
        public GetAllProductsActivedQueryHandler(IProductRepository productRepository,
        ...
        public async Task<Result<List<ProductFullDTO>>> Handle(GetAllProductsActivedQuery request, CancellationToken cancellationToken)
        {
            ...
            var products = productRepository.FindAll(x => x.IsActived == 1 && category.Ids.Contains(x.CategoryId)).ToList();
