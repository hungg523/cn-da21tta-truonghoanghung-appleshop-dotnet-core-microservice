...
using AppleShop.auth.queryApplication.Validator.Order;
...
namespace AppleShop.auth.queryApplication.Handler.Order
{
    public class GetOrderByUserIdQueryHandler : IRequestHandler<GetOrderByUserIdQuery, Result<List<OrderFullDTO>>>
    {
        private readonly IAuthRepository orderRepository;
        private readonly IRequestClient<GetAllCartItemByUserIdEvent> requestClient;
        private readonly IOrderItemRepository orderItemRepository;

        public GetOrderByUserIdQueryHandler(IAuthRepository orderRepository, IRequestClient<GetAllCartItemByUserIdEvent> requestClient, IOrderItemRepository orderItemRepository)
        {
        ...
        public async Task<Result<List<OrderFullDTO>>> Handle(GetOrderByUserIdQuery request, CancellationToken cancellationToken)
        {
            ...
            if (orders is null) AppleException.ThrowNotFound(typeof(Entities.Auth));
